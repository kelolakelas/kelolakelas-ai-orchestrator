    ...
    if (method === 'GET') {
      ...
      if (segments.length === 2 && segments[0] === 'tasks') {
        ...
        return sendJson(response, 200, {
          task,
          workUnits: await options.operator.listWorkUnits(taskId),
          attempts: await options.operator.listAttempts(taskId),
          actions: await options.operator.listActions(taskId),
        });
      }
